...
export default (() => {
  ...
  if (INITIAL_SETTINGS.disableBeforeInput) {
    // @ts-expect-error: getTargetRanges is not always defined on InputEvent in all browsers
    delete window.InputEvent.prototype.getTargetRanges;
  }

  // @ts-expect-error: EXCALIDRAW_ASSET_PATH is not typed on window object
